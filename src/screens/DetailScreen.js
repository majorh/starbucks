...
export default function DetailScreen({ route, navigation }) {
  ...
  const openStarbucks = () => {
    Linking.canOpenURL('starbucks://').then((supported) => {
      Linking.openURL(
        supported
          ? 'starbucks://'
    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      >
        ...
        <TouchableOpacity style={styles.btnPrimary} onPress={openStarbucks} activeOpacity={0.85}>
          <Text style={styles.btnPrimaryText}>☕  Open Starbucks App</Text>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  milkBoxText: { fontSize: 15, color: colors.cream, lineHeight: 22 },
  ...
  warnBox: {
    ...
    borderLeftWidth: 3,
    borderLeftColor: 'rgba(255,180,0,0.5)',
    paddingVertical: 12,
